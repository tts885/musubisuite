...
export default function OcrDocumentListPage() {
  ...
  const getConfidenceBadge = (confidence: number) => {
    if (confidence >= 0.95) {
      ...
      return <Badge variant="secondary" className="bg-yellow-500">{(confidence * 100).toFixed(0)}%</Badge>
    } else {
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              ...
              全 {filteredStats.total} 件 | 完了 {filteredStats.completed} 件 | 処理待ち {filteredStats.pending} 件
            </p>
          </div>
          <Button onClick={() => {
