...
export default function Layout({ showHeader = true }: LayoutProps) {
  ...
  const getBreadcrumbs = () => {
    ...
    // 詳細ページの場合（例: /projects/:id）
    if (!currentName && location.pathname.startsWith('/projects/')) {
      return [
        { path: '/projects', name: '案件一覧' },
        { path: location.pathname, name: '案件詳細' }
      ]
    }
    
    // トップレベルのページの場合は現在のページのみ表示
    return currentName ? [{ path: location.pathname, name: currentName }] : [{ path: '/', name: 'ダッシュボード' }]
  }
      ...
      >
        <div className="flex flex-col h-full">
          ...
          <div className={`h-16 border-b border-sidebar-border flex items-center ${sidebarCollapsed ? 'justify-center' : 'px-5 gap-3'}`}>
            ...
            {!sidebarCollapsed && (
              <span className="font-bold text-lg whitespace-nowrap text-sidebar-foreground">Musubi Suite</span>
            )}
          </div>
          ...
          <div className="p-4 border-t border-sidebar-border">
            {!sidebarCollapsed && (
              <div className="text-xs text-sidebar-foreground/50 space-y-1">
                <p className="font-medium">Musubi Suite v1.0</p>
