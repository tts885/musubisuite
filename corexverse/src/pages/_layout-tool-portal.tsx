...
const navItems = [
  ...
  { 
    path: "/tool-portal/config", 
    label: "設定", 
    description: "環境設定の管理",
    icon: Settings 
  },
  { 
    path: "/tool-portal/automation", 
    label: "自動化", 
    description: "CI/CDとワークフロー",
    icon: Zap 
  },
  { 
    path: "/tool-portal/database", 
    label: "データベース", 
    description: "DB管理とクエリ",
    icon: Database 
  },
  { 
    path: "/tool-portal/dataverse-settings", 
    label: "Dataverse設定", 
    description: "テーブルとスキーマ管理",
    icon: Database 
  },
]
...
export default function ToolPortalLayout({ showHeader = true }: LayoutProps) {
  ...
  const getBreadcrumbs = () => {
    const routeNames: Record<string, string> = {
      '/tool-portal': 'ツールPortal',
      '/tool-portal/config': '設定',
      '/tool-portal/automation': '自動化',
      '/tool-portal/database': 'データベース',
      '/tool-portal/dataverse-settings': 'Dataverse設定',
    }
    ...
    // サブページの場合は親ページも表示
    if (currentName && location.pathname !== '/tool-portal') {
      return [
        { path: '/tool-portal', name: 'ツールPortal' },
        { path: location.pathname, name: currentName }
      ]
    }
    
    // トップレベルのページの場合は現在のページのみ表示
    return currentName ? [{ path: location.pathname, name: currentName }] : [{ path: '/tool-portal', name: 'ツールPortal' }]
  }
    ...
    <div className="h-screen flex overflow-hidden bg-background">
      ...
      >
        <div className="flex flex-col h-full">
          ...
          <div className={`border-b border-sidebar-border flex items-center ${sidebarCollapsed ? 'h-16 justify-center' : 'h-16 px-4 gap-3'}`}>
            ...
            {!sidebarCollapsed && (
              <>
                ...
                <h1 className="text-lg font-bold text-foreground">ツールPortal</h1>
              </>
            )}
          </div>
          ...
          <div className="px-4 py-3 border-t bg-card text-xs text-muted-foreground flex items-center gap-4">
            {!sidebarCollapsed && (
              <span>Tool Portal: 開発ツールと自動化</span>
            )}
          </div>
        </div>
      </aside>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <main className="flex-1 overflow-auto bg-background flex flex-col">
          ...
          <PageFooter
            leftContent={<span>© 2025 CoreXverse - ツールPortal</span>}
